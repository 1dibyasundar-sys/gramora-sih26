...
import { SetUserStatusSchema } from '@/server/domain/admin';
 ...
 * Explicit administrative command for updating a user's account status (active/suspended) with audit logging.
 */
export async function POST(
  ...
  try {
    ...
    const validated = SetUserStatusSchema.parse(body);

    const updatedUser = await adminService.setUserStatus(user, targetUid, validated, ctx.requestId);
